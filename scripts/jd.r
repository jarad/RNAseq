...
c = apply(d, 2, quantile, prob=0.75)
d = d[1:100,]
...
d.lookup = data.frame(variable=names(c), c=c)
dm = join(dm, d.lookup, by='variable')
...
inits = list(alpha=log(aggregate(value~gene,dm,mean)$value+.1)/mean(c))
